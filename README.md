...
```
데이터 파악 → EDA → 전처리 → ML/Modeling → DL → XAI → Retention Strategy → 배포
```
...
> `churn_events`는 이탈 이후 데이터이므로 예측 모델 학습에서 의도적으로 제외 → **데이터 누수 방지**  
> `feature_usage`는 subscription 단위이므로 account 단위로 집계 후 조인
...
### 핵심 인사이트
- 이탈 고객은 비이탈 고객 대비 **사용량(feature usage)이 낮고, 오류 경험이 많음**
- `satisfaction_score` 결측률 약 41% → 결측 자체가 불만족 신호일 가능성
...
- 단순 accuracy가 아닌 **이탈 탐지 관점의 Recall** 중심으로 평가
- SHAP 해석 가능성이 높은 **Random Forest** 기준 최종 선택
- Precision / Recall 트레이드오프를 고려한 threshold 기반 운영 기준 수립
...
| 대상 고객군 | 전략 |
|------------|------|
| 사용량 저하 고객 | 선제적 케어 및 온보딩 재지원 |
| 오류 경험 고객 | 우선 대응 및 기술 지원 강화 |
| 고위험 고객군 | 세분화 운영 및 전담 CSM 배정 |
| threshold 기반 | 운영 기준 수립 및 알림 자동화 |
